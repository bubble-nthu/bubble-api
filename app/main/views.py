...
def index():
    ...
    pagination = query.order_by(Post.timestamp.desc()).paginate(
        page, per_page=current_app.config['BUBBLE_POSTS_PER_PAGE'], error_out=False)
...
def show_followed():
    resp = make_response(redirect(url_for('.index')))
    resp.set_cookie('show_followed', '1', max_age=30*24*60*60) # 30 dats
    return resp
...
def post(id):
    ...
    if page == -1:
        page = (post.comments.count() - 1) // \
            current_app.config['BUBBLE_COMMENTS_PER_PAGE'] + 1
    pagination = post.comments.order_by(Comment.timestamp.asc()).paginate(
        page, per_page=current_app.config['BUBBLE_COMMENTS_PER_PAGE'],
        error_out=False)
...
def followers(username):
    ...
    pagination = user.followers.paginate(
        page, per_page=current_app.config['BUBBLE_FOLLOWERS_PER_PAGE'],
        error_out=False)
